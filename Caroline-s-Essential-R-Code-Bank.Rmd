...
#Parametric t test for when your data follows a normal distribution
...
#Do an ANOVA when you have multiple independent categorical variables that you think explain your dependent variable
#Be careful here, the anova() call does something a bit different than aov(), which does the more simple ANOVA we all know and love.

#We would want to do an ANOVA when you are really interested in comparing the MEANS between categorical groups. Note that up next is linear modeling (including mixed linear modeling) which gives you the EFFECT SIZE of your independent variables on your dependent (response) variable.
...
TukeyHSD(aov_resp_e_coli, "temp") #this tells you that you want to see pairwise comparisons of temperature (or whatever independent variable you're interested in)
```
...
#What you need to run the model / some important notes
  #independence of PREDICTOR variables (e.g. you would have to think about whether you could add both temperature and oxygen as predictor variables, because there is a non-independent relationship between temp and oxygen--colder water holds more oxygen)
  #assumes linearity (normality) -- you can always transform to make it normal, but you may also want to consider going with a generalized linear model where you use a "link" function. More on that later.
  #have a think about the class of your independent variables -- for example, light/dark will likely be a categorical variable. However, if you have 1%, 50% and 100% light you will likely want to make it a numerical type so that R knows that 1% is less than 50%, etc.
  #you may need to scale parameters (e.g. make everything in the metric system). Note this!

#The sign of a linear regression coefficient (called an estimate in the output) tells you whether there is a positive or negative correlation between each independent variable and the dependent variable. It is the slope by which a change in x corresponds to a change in y.
...
#A NOTE ON REFERENCE CLASS AND OUTPUT
#Rs default option is to give you the difference based on the reference class of each predictor variable
#Reference grup is always the first group in a factor unless you change the setting
```
...
r2(mm_resp_2) #r2 tells us how much variance in our data is explained by our model. Marginal id variance is the amount of variance explained just by fixed factors (independent variables), and conditional includes variance explained by random factor. You can interpret these as percentages.
